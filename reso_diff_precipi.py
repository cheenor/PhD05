...
MY500=1202
nt=2880
CASE=['PRDCTR_H','MLYRCTR_H', 'NPCCTR_H',
...
for ig in range(0,nga):
    ...
    rainobs.append(rain)
###############################################################################
pre3kma=np.zeros(shape=(nga,nt),dtype=float)
pbl3kma=np.zeros(shape=(nga,nt),dtype=float)
pre500ma=np.zeros(shape=(nga,nt),dtype=float)
pbl500ma=np.zeros(shape=(nga,nt),dtype=float)
pre3kma3h=np.zeros(shape=(nga,nt/12),dtype=float)
pbl3kma3h=np.zeros(shape=(nga,nt/12),dtype=float)
pre500ma3h=np.zeros(shape=(nga,nt/12),dtype=float)
pbl500ma3h=np.zeros(shape=(nga,nt/12),dtype=float)
for ig in range(0,nga):
    for it in range(0,nt):
        pre3kma[ig,it]=pre3k[ig,1:MY3k-1,it].mean()
        pbl3kma[ig,it]=pbl3k[ig,1:MY3k-1,it].mean()
        pre500ma[ig,it]=pre500[ig,1:MY500-1,it].mean()
        pbl500ma[ig,it]=pbl500[ig,1:MY500-1,it].mean()
    for itt in range(0,nt/12):
        ...
        for n in range(nt1,nt2):
            pre3kma3h[ig,itt]= pre3kma3h[ig,itt]+ pre3kma[ig,n]/dn
            pbl3kma3h[ig,itt]= pbl3kma3h[ig,itt]+ pbl3kma[ig,n]/dn
...
for ig in range(0,nga):
    ...
    print jr,jc
    xdate=alldate[ig]
    ax[jr,jc].plot(xxx[0:ns],pre3kma3h[ig,0:ns],color=color_cycle[0],lw=wd[0],label="CRM 3000m") 
...
plt.show()                     
plt.savefig(dirpic+'resolu_ALLCASE_rainfall_Color.png',dpi=300)          
plt.show()
plt.close()