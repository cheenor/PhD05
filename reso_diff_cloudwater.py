...
xdat=range(0,nt)  
###############################################################################
rawdata3k=np.zeros(shape=(nga,2,nz,nt),dtype=float)
cloudpath3k=np.zeros(shape=(nga,5,2,nt),dtype=float)
rawdata500=np.zeros(shape=(nga,2,nz,nt),dtype=float)
cloudpath500=np.zeros(shape=(nga,5,2,nt),dtype=float)
ntt=nt/12
rawdata3k3h=np.zeros(shape=(nga,2,nz,ntt),dtype=float)
cloudpath3k3h=np.zeros(shape=(nga,5,2,ntt),dtype=float)
...
for iga in range(0,nga):
    ...
    fpath3k=dirin3k+name3k+'_ALLTYPES_CLOUDWATER_PATH.TXT'
    iskp=0
    ondedim3k=readAscii(fpath, iskp)
    ...
    for it in range(0,nt):
        ...
        for iz in range(0,nz):
            ikk=iks+iz               
            rawdata3k[iga,0,iz,it]=ondedim3k[ikk]               
            rawdata3k[iga,1,iz,it]=ondedim3k[ikk+nz]
            rawdata500[iga,0,iz,it]=ondedim500[ikk]               
            rawdata500[iga,1,iz,it]=ondedim500[ikk+nz]
        iks=iks+2*nz         
        for itk in range(0,5):
            ikk=iks+itk
            cloudpath3k[iga,itk,0,it]=ondedim3k[ikk]
            cloudpath500[iga,itk,0,it]=ondedim500[ikk]
            if ondedim3k[ikk]<0:
                cloudpath3k[iga,itk,0,it]=0.
            if ondedim500[ikk]<0:
                cloudpath500[iga,itk,0,it]=0.
        iks=iks+5
        for itk in range(0,5):
            ikk=iks+itk
            cloudpath3k[iga,itk,1,it]=ondedim3k[ikk]
            if ondedim3k[ikk]<0:
                cloudpath3k[iga,itk,1,it]=0.
            cloudpath500[iga,itk,1,it]=ondedim500[ikk]
            if ondedim500[ikk]<0:
                cloudpath500[iga,itk,1,it]=0.
        for itk in range(0,2):
            for iz in range(0,nz):
                if rawdata3k[iga,itk,iz,it]<0:
                    rawdata3k[iga,0,iz,it]=None
                if rawdata500[iga,itk,iz,it]<0:
                    rawdata500[iga,0,iz,it]=None
        print iks
    for itt in range(0,ntt):
        ...
        for iz in range(0,nz):
            for ik in range(0,2):
                rawdata3k3h[iga,ik,iz,itt]=0.
                rawdata5003h[iga,ik,iz,itt]=0.
                for it1 in range(nnt1,nnt2):                    
                    rawdata3k3h[iga,ik,iz,itt]=rawdata3k3h[iga,ik,iz,itt] \
                        +rawdata3k[iga,ik,iz,it1]/dnn 
                    rawdata5003h[iga,ik,iz,itt]=rawdata5003h[iga,ik,iz,itt] \
                        +rawdata500[iga,ik,iz,it1]/dnn                    
        for itk in range(0,5):
            for ik in range(0,2):
                cloudpath3k3h[iga,itk,ik,itt]=0.
                cloudpath5003h[iga,itk,ik,itt]=0.
                for it1 in range(nnt1,nnt2):                   
                    cloudpath3k3h[iga,itk,ik,itt]=cloudpath3k3h[iga,itk,ik,itt] \
                        +cloudpath3k[iga,itk,ik,it1]/dnn
                    cloudpath5003h[iga,itk,ik,itt]=cloudpath5003h[iga,itk,ik,itt] \
                        +cloudpath500[iga,itk,ik,it1]/dnn
...
for iga in range(0,nga):
    ...
    plt.subplot(3,2,ij)
    zdat=rawdata5003h[iga,1,:,:]-rawdata3k3h[iga,1,:,:]
